...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `[${executionId}] Generated $${totalAmount.toFixed(2)} from ${totalTransactions} transactions`
    );
...
async function generateContentLicensing(supabase: any, executionId: string) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const content_type = contentTypes[Math.floor(Math.random() * contentTypes.length)];
...
async function generateDataMonetization(supabase: any, executionId: string) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const data_type = dataTypes[Math.floor(Math.random() * dataTypes.length)];
